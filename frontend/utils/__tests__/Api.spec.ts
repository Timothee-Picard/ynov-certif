...
describe('authApi', () => {
    ...
    it('register KO', async () => {
        ...
        await expect(
            authApi.register({ username: 'Tim', email: 'a@b.com', password: 'x' }),
...
describe('todoListApi', () => {
    ...
    it('createTodoList KO', async () => {
        ...
        await expect(
            todoListApi.createTodoList({ name: 'X', description: '', color: '#3B82F6' }),
        ).rejects.toThrow('No')
    })
...
describe('taskApi', () => {
    ...
    it('createTask OK', async () => {
        ...
        mockFetchOnce({ ok: true, status: 201, json: created })
        const res = await taskApi.createTask('l1', {
            title: 'New',
            description: 'D',
            priority: 'high',
            isCompleted: false,
