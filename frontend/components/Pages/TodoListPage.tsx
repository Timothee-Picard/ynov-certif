...
export function TodoListPage({ id }: TodoListPageProps) {
	...
	if (!todoList) {
		return (
			<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" data-testid="list-not-found">
		...
		>
			...
			<div className="mb-8" data-testid="todolist-header">
				<div className="flex items-center space-x-4 mb-4">
					...
					/>
					<div>
						<h1 className="text-2xl font-bold text-gray-900" id={titleId} data-testid="todolist-title">
							{todoList.name}
						</h1>
                ...
                {completedCount}/{tasks.length} tâches terminées ({Math.round(progress)}%)
              </span>
						</div>
			...
			{showForm && (
				<div data-testid="task-form">
					<TaskForm task={editingTask} onSave={handleSaveTask} onCancel={handleCancelForm} loading={formLoading} />
				</div>
			)}
		</main>
	);
