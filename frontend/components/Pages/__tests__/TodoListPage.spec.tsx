...
jest.mock('@/hooks/useTasks', () => ({
	useTasks: () => ({
		tasks: [
			{ id: 't1', title: 'Task 1', description: 'Desc 1', isCompleted: false, createdAt: new Date().toISOString() },
			{ id: 't2', title: 'Task 2', description: 'Desc 2', isCompleted: true, createdAt: new Date().toISOString() },
		],
		loading: false,
		error: null,
...
jest.mock('@/components/Tasks/TaskItem', () => ({
	TaskItem: ({ task, onToggleComplete, onEdit, onDelete }: any) => (
		<div data-testid={`task-${task.id}`}>
			...
			<button data-testid={`toggle-${task.id}`} onClick={() => onToggleComplete(task.id)}>toggle</button>
			<button data-testid={`edit-${task.id}`} onClick={() => onEdit(task)}>edit</button>
			<button data-testid={`delete-${task.id}`} onClick={() => onDelete(task.id)}>delete</button>
		</div>
	),
}));
...
jest.mock('@/components/Tasks/TaskForm', () => ({
	TaskForm: ({ task, onSave, onCancel, loading }: any) => (
		<div data-testid="task-form-content" aria-busy={loading ? 'true' : 'false'}>
			...
			<button data-testid="task-form-cancel" onClick={onCancel}>cancel</button>
		</div>
	),
}));
...
describe('TodoListPage', () => {
	...
	it('ouvre le formulaire en mode édition et sauvegarde', async () => {
		...
		expect(mockUpdateTask).toHaveBeenCalledWith('t1', expect.objectContaining({ title: 'Saved task' }));
	});
