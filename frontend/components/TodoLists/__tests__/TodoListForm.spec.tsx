...
describe('TodoListForm', () => {
	...
	it('pré-remplit les champs en mode édition et sélectionne la bonne couleur', () => {
		...
		expect(screen.getByTestId('todolistform-description')).toHaveValue('Acheter fruits et légumes');

		expect(screen.getByTestId('todolistform-color-#EF4444')).toHaveAttribute('aria-checked', 'true');
	});
	...
	it('permet de changer la couleur (radiogroup)', async () => {
		...
		expect(oldBtn).toHaveAttribute('aria-checked', 'true');
		expect(newBtn).toHaveAttribute('aria-checked', 'false');
