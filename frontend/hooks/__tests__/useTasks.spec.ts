...
describe('useTasks', () => {
	...
	it('createTask: ajoute la tâche (et appelle API avec listId)', async () => {
		...
		const created = makeTask({ id: 't2', title: payload.title, description: payload.description, priority: 'high' });
	...
	it('updateTask: remplace la tâche par la version mise à jour', async () => {
		const initial = [makeTask({ id: 't1', title: 'Old' }), makeTask({ id: 't2', title: 'Keep' })];
		getTasks.mockResolvedValue(initial);
